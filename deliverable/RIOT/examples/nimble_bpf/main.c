...
static uint8_t _bpf_stack[512] = { 0 };
...
static bpf_t _temp_bpf = {
    .application = temp_sens_bin,
    .application_len = temp_sens_bin_len,
    .stack = _bpf_stack,
                        ...
                        struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    int64_t result;
    uint64_t type = SAUL_SENSE_TEMP;

    int res = bpf_execute_ctx(&_temp_bpf, &type, sizeof(type), &result);
    ...
    res = os_mbuf_append(ctxt->om, &fmt_result, sizeof(fmt_result));
    return (res == 0) ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
}
