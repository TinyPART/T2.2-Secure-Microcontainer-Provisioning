                        ...
                        struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    wasm_module_inst_t temperature_instance = NULL;

    char error_buf[128];
    if (!(temperature_instance = wasm_runtime_instantiate(temperature_module, 8 * 1024,
    ...
    result = iwasm_instance_exec_main(temperature_instance, argc, argv);

    wasm_runtime_deinstantiate(temperature_instance);

    int16_t fmt_result = (intptr_t)result;
    printf("Resulting temperature %"PRIi16"\n", fmt_result);

    result = os_mbuf_append(ctxt->om, &fmt_result, sizeof(fmt_result));
